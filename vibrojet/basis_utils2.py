...
def legendre(x, l, m):
    ...
    f = (
        (-1 * sin_x) ** m
        * legval(np.asarray(np.cos(x)), legder(coef, m))
        * jnp.sqrt(sin_x)
    )
    return f[m:, :].T
...
def fourier(x, n):
    f = np.sin(
        np.ceil(n[None, :] / 2) * x[:, None] + np.pi / 2 * np.mod(n[None, :] + 1, 2)
    ) / np.sqrt(np.pi)
    f[:, 0] /= np.sqrt(2)
    return f
...
class ContrBasis:
    def __init__(
        ...
        for ibatch in range(n_batches):
            ...
            index_batch = jnp.arange(
                ibatch * batch_size_, np.min([(ibatch + 1) * batch_size_, nbas**2])
            )
            m_ind_flat_batch = []
            ...
            gme[index_batch] = compute_gme(
                bas_list, m_ind_flat_batch, coupl_ind, coo_ind, me0, Gmat_coefs
            )
            end_time = time.time()
        ...
        if store_int:
            ...
            for ibatch in range(n_batches):
                ...
                index_batch = np.arange(
                    ibatch * batch_size_, min((ibatch + 1) * batch_size_, nbas**2)
                )
                ...
                v_bra = v[index_bra, :]
                v_ket = v[index_ket, :]
                v_braket = jnp.einsum("qi,qj->qij", v_bra, v_ket).reshape(
                    (len(index_batch), -1)
                )
                ...
                end_time = time.time()
                print(
                    "iteration time compute objects to store:",
